...
int run_service() {
    ...
    auto config = securecloud::audit::AuditConfig::load(env_source, config_errors);
...
int main() noexcept {
    try {
        ...
        std::cerr << "[SecureCloud] [audit] Unhandled exception: " << ex.what() << "\n";
        return 1;
    } catch (...) {
        std::cerr << "[SecureCloud] [audit] Unknown fatal error occurred\n";
        return 1;
    }
}
